...
{
    // 入力レイアウトを生成.
    if ( !OnCreateIL( pDevice, pBlob->GetBufferPointer(), pBlob->GetBufferSize() ) )
    {
...
void Mesh::Draw( ID3D11DeviceContext* pDeviceContext )
{
    ...
    {
        // マテリアル設定処理.
        OnUpdateMaterial( pDeviceContext, i );

        // 描画キック処理.
        OnDrawKick( pDeviceContext, i );
    }
...
bool Mesh::DefOnCreateIB( ID3D11Device* pDevice, const asdx::ResMesh& mesh )
{
    ...
    ZeroMemory( &desc, sizeof( D3D11_BUFFER_DESC ) );
    desc.Usage          = D3D11_USAGE_DEFAULT;
    desc.ByteWidth      = sizeof( u32 ) * mesh.GetIndexCount();
...
//      マテリアル設定の処理です.
//-------------------------------------------------------------------------------------------
void Mesh::DefOnUpdateMaterial( ID3D11DeviceContext* pDeviceContext, const u32 index )
{
    u32 id = m_pSubset[ index ].MaterialID;
...
//      デフォルトの描画キック処理です.
//-------------------------------------------------------------------------------------------
void Mesh::DefOnDrawKick( ID3D11DeviceContext* pDeviceContext, const u32 index )
{
    // 描画キック処理.
...
//      入力レイアウト生成時の処理です.
//-------------------------------------------------------------------------------------------
bool Mesh::OnCreateIL( ID3D11Device* pDevice, const void* shaderByteCode, const u32 byteCodeLength )
{
...
//      マテリアル設定の処理です.
//-------------------------------------------------------------------------------------------
void Mesh::OnUpdateMaterial( ID3D11DeviceContext* pDeviceContext, const u32 index )
{
    DefOnUpdateMaterial( pDeviceContext, index );
}

//-------------------------------------------------------------------------------------------
//      描画キック時の処理です.
//-------------------------------------------------------------------------------------------
void Mesh::OnDrawKick( ID3D11DeviceContext* pDeviceContext, const u32 index )
{
    DefOnDrawKick( pDeviceContext, index );
}
