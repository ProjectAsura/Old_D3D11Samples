...
bool Application::InitD3D()
{
    ...
    // レンダーターゲットを生成.
    if ( !m_RT.CreateFromBackBuffer( m_pDevice, m_pSwapChain ) )
    {
    ...
    // 深度ステンシルバッファを生成.
    if ( !m_DST.Create( m_pDevice, desc ) )
    {
    ...
    // ラスタライザーステートの設定.
    {
        ...
        // ラスタライザーステートを生成.
        hr = m_pDevice->CreateRasterizerState( &rd, &m_pRS );
        if ( FAILED( hr ) )
        {
            DLOG( "Error : ID3D11Device::CreateRasterizerState() Failed." );
            return false;
        }
        SetDebugObjectName( m_pRS, "asdxApp" );
    ...
    // 深度ステンシルステートを取得.
    {
        m_pDeviceContext->OMGetDepthStencilState( &m_pDSS, &m_StencilRef );
    }

    // ブレンドステートを取得.
    {
        m_pDeviceContext->OMGetBlendState( &m_pBS, m_BlendFactor, &m_SampleMask );
    }
...
void Application::TermD3D()
{
    ...
    // スワップチェインを解放.
    ASDX_RELEASE( m_pSwapChain );

    // デバイスコンテキストを解放.
    ASDX_RELEASE( m_pDeviceContext );

    // デバイスを解放.
    ASDX_RELEASE( m_pDevice );
}
...
void Application::MainLoop()
{
    ...
    FrameEventParam frameEventParam;
    ...
    while( WM_QUIT != msg.message )
    {
        ...
        else
        {
            ...
            frameEventParam.pDeviceContext  = m_pDeviceContext;
            frameEventParam.FPS             = m_FPS;
            frameEventParam.Time            = time;
            frameEventParam.ElapsedTime     = elapsedTime;
            frameEventParam.IsStopDraw      = m_IsStopRendering;

            // フレーム遷移処理.
            OnFrameMove( frameEventParam );

            // 描画停止フラグが立っていない場合.
            if ( !IsStopRendering() )
            {
                // フレーム描画処理.
                OnFrameRender( frameEventParam );
    ...
    frameEventParam.pDeviceContext = nullptr;
}
...
//      フレーム遷移時の処理.
//---------------------------------------------------------------------------------------------
void Application::OnFrameMove( FrameEventParam& param )
{
...
//      フレーム描画字の処理.
//---------------------------------------------------------------------------------------------
void Application::OnFrameRender( FrameEventParam& param )
{
    ASDX_UNUSED_VAR( param );
